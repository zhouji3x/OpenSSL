...
function main() {
  ...
  declare -r command=$1
  shift || usage "No command specified. Try import."
  if [ "$command" = "import" ]; then
    declare -r tar=$1
    shift || usage "No tar file specified."
    import $tar
  else
    usage "Unknown command specified $command. Try import."
  fi
}
...
function generate_config_mk() {
  declare -r output="$1"
  declare -r prefix="$2"
  declare -r all_archs="x86 x86_64"
...
"
    ...
    print_vardef_with_prefix_in_mk common_c_includes vendor/intel/external/openssl/ $common_includes
...
function applypatches () {
  ...
  git init .
  git add .
  git commit -s -m "Initial commit" >/dev/null
